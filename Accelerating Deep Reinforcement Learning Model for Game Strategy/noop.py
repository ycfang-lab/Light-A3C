...
def run(path=""):
    ...
    while T < args.T_max:
        if done:
            state, done = env.reset(), False
        next_state, reward, done = env.step(random.randint(0, action_space - 1))  # Step
        if args.reward_clip > 0:
            reward = max(min(reward, args.reward_clip), -args.reward_clip)  # Clip rewards
        mem.append(state, 0, reward, done)  # Append transition to memory
        T += 1
        ...
        if T >= args.learn_start:
            ...
            if T % args.evaluation_interval == 0:
                ...
                for _ in range(args.evaluation_episodes):
                    while True:
                        if done:
                            state, reward_sum, done = tenv.reset(), 0, False
                        state, reward, done = tenv.step(random.randint(0, action_space - 1))  # Step
                        if args.render:
                            tenv.render()
                        if done:
                            break
...
if __name__ == "__main__":
    ...
    run(os.path.join("./results", args.game, "./noop"))
