...
class Agent:
    def __init__(self, args, env):
        ...
        self.online_net = StrandDQN(args, self.action_space).to(device=args.device)
        if args.model and os.path.isfile(args.model):
            # Always load tensors onto CPU by default, will shift to GPU if necessary
            self.online_net.load_state_dict(torch.load(args.model, map_location='cpu'))
        self.online_net.train()

        self.target_net = StrandDQN(args, self.action_space).to(device=args.device)
