...
from bin_utils import BinOp
from dqns import BinaryDQN
...
class Agent:
    def __init__(self, args, env):
        ...
        self.binary_online = BinOp(self.online_net)
        self.binary_target = BinOp(self.target_net)
    ...
    def act(self, state):
        ...
        with torch.no_grad():
            # self.binary_online.binarization()
            acts = self.online_net(state.unsqueeze(0))
            # self.binary_online.restore()
            # self.online_net.train()
            return (acts * self.support).sum(2).argmax(1).item()
    ...
    def learn(self, mem):
        ...
        with torch.no_grad():
            ...
            self.binary_target.binarization()
            pns = self.target_net(next_states)  # Probabilities p(s_t+n, ·; θtarget)
            self.binary_target.restore()
            pns_a = pns[range(
        ...
        self.binary_online.restore()
        self.binary_online.updateBinaryGradWeight()
        self.optimiser.step()
    ...
    def evaluate_q(self, state):
        ...
        with torch.no_grad():
            self.binary_online.binarization()
            q = (self.online_net(state.unsqueeze(0)) * self.support).sum(2).max(1)[0].item()
            self.binary_online.restore()
            self.online_net.train()
            return q
