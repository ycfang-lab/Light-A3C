...
def run(agent, path=""):
    ...
    if args.evaluate:
        ...
        avg_reward, avg_Q = test(args, 0, dqn, val_mem,path, evaluate=True)  # Test
        log('Avg. reward: ' + str(avg_reward) + ' | Avg. Q: ' + str(avg_Q),path)
    else:
        # Training loop
        dqn.train()
        T, done = 0, True
        while T < args.T_max:
            ...
            if T % args.log_interval == 0:
                log('T = ' + str(T) + ' / ' + str(args.T_max),path)
            ...
            if T >= args.learn_start:
                ...
                if T % args.evaluation_interval == 0:
                    dqn.eval()  # Set DQN (online network) to evaluation mode
                    avg_reward, avg_Q = test(args, T, dqn, val_mem, path)  # Test
                    log('T = ' + str(T) + ' / ' + str(args.T_max) + ' | Avg. reward: ' + str(
                        avg_reward) + ' | Avg. Q: ' + str(avg_Q),path)
...
if __name__ == "__main__":
    ...
    if not os.path.exists("./results"):
        os.mkdir("./results")
    if not os.path.exists(os.path.join("./results", args.game)):
        os.mkdir(os.path.join("./results", args.game))
    from agents.binary_agent import Agent

    run(Agent, os.path.join("./results", args.game, "./binary_cnn_liner"))
